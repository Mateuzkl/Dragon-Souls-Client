...
local voidelements = dofile('htmlcssparser/voidelements.lua')
--}}}
...
local function parse(text, limit) -- {{{
    ...
    if not opts.keep_danger_placeholders then -- {{{ little speedup by cost of potential parsing breakages
        ...
        text = text:gsub("(=[%s]-)" .. -- only match attr.values, and not random strings between two random apostrophs
                         "(%b'')",
                         function(...) return g(2, ...) end)
                  :gsub("(=[%s]-)" .. -- same for "
                        "(%b"")",
                        function(...) return g(2, ...) end)                            -- Escape "<"/">" inside attr.values (see issue #50)
                  :gsub("(<" ..                  -- Match "<",
    ...
    while true do        -- MainLoop {{{
        ...
        openstart, tpos, name = root._text:find("<" .. -- an uncaptured starting "<"
        ...
        while true do       -- TagLoop {{{
            ...
            start, apos, k, zsp, eq, zsp, quote = tagst:find("%s+" .. -- some uncaptured space
